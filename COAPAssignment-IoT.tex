...
\title[PCP1276 - INTERNET OF THINGS - CoAP]{Constrained Application Protocol(CoAP)}

\author{Subalakshmi Shanthosi S}

\address{M.E(CSE) I Year , Reg No:186001008}
\ead{subalakshmi1808@cse.ssn.edu.in}
\vspace{10pt}
\begin{indented}
\item[]May 14th 2019
\end{indented}
...
\begin{abstract}
	CoAP is an internet utility protocol for constrained gadgets. It is designed to enable simple, constrained devices to join IoT through constrained networks having low bandwidth availability. This protocol is primarily used for \textbf{machine-to-machine (M2M) communication} and is particularly designed for IoT systems that are based on HTTP protocols. CoAP follows Asynchronous message exchange patterns.CoAP is a \textbf{service layer protocol} that is intended for use in resource-constrained internet devices, such as wireless sensor network nodes.
\end{abstract}
...
\section{Introduction: Constrained Application Protocol }
Main Features of CoAP are:
\begin{itemize}
	\item Web protocol used in M2M with constrained requirements.
	\item Asynchronous message exchange
	\item Low overhead and very simple to parse
	\item URI and content-type support
	\item Proxy and caching capabilities
	
\end{itemize}
More precisely a concised version of HTTP.
Made of two different layers : \textbf{Message and Request/Response}
\begin{itemize}
\item Message layer  : UDP and asynchronous messages.
\item Requests/Response layer - HTTP messages handling.
\end{itemize} 

\subsection{CoAP message types}
\begin{itemize}
	\item Confirmable
...
\subsection{Structure Elements of CoAP}
\begin{itemize}
	\item Endpoint : 
	    An Entity. Mostly a host.
	
	\item Sender :
	    An Entity which sends messages.
	
	\item Client: 
		An Entity that sends a request and the destination of the response
   
    \item Server: 
    	An entity that receives a request from a client and sends back a response to the client
    \end{itemize}
\normalsize
